  ...
  return (
    <main className="org org-page">
      ...
      <div className="org-subnav" role="navigation" aria-label="Vues">
        <Link className={'org-subnav__link' + (vue === 'attendus' ? ' is-active' : '')} href="?vue=attendus" aria-current={vue === 'attendus' ? 'page' : undefined}>À entrer ({attendus})</Link>
        <Link className={'org-subnav__link' + (vue === 'entres' ? ' is-active' : '')} href="?vue=entres" aria-current={vue === 'entres' ? 'page' : undefined}>Déjà entrés ({entres})</Link>
      </div>
      <form className="org-filters glass" method="get">
        ...
        <div className="org-filters__actions"><button className="btn btn--amber">Rechercher</button>{q && <Link className="btn btn--outline" href={`?vue=${vue}`}>Effacer</Link>}</div>
      </form>
      ...
      <p><Link className="btn btn--outline" href={`/organisateur/evenements/${slug}?onglet=scan`}>Ouvrir le scan</Link></p>
    </main>
  );
}
