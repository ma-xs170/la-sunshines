  ...
  return (
    <main className="org org-page">
      ...
      <section className="glass ef-card"><h2>Résumé</h2>
        ...
        <p>Sous-total {formatEuro(Number(o.subtotal_cents))} · frais {formatEuro(Number(o.fee_cents))} · <strong>total {formatPrice(Number(o.total_cents))}</strong>{Number(o.refunded_cents) > 0 && ` · remboursé ${formatEuro(Number(o.refunded_cents))}`}</p>
      ...
      <section className="glass ef-card"><h2>Billets</h2>
        ...
        <ul className="ef-list">{d.items.map((i, k) => <li key={k}><span>{i.quantity} × {i.tier}</span><span>{formatPrice(i.unit_price_cents)}</span></li>)}</ul></section>
