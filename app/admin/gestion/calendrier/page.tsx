...
export default async function AdminCalendarPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return (<><h1 className="org-head__title">Calendrier · {REGION_LABEL[region]}</h1>{!r.ok && <p className="admin-error" role="alert">{r.message}</p>}<RegionalCalendar region={region} events={r.ok ? r.data : []} admin names={names} /></>);
}
