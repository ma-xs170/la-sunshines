...
export default async function PublicationsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return (
    <>
      ...
      <div className="org-subnav" role="navigation" aria-label="Statuts">{TABS.map(([k, v]) => <a key={k} href={`?statut=${k}`} className={'org-subnav__link' + (st === k ? ' is-active' : '')} aria-current={st === k ? 'page' : undefined}>{v}{k === 'pending' && r.ok && r.data.counts.pending > 0 && <b className="oside__badge" style={{ marginLeft: 6 }}>{r.data.counts.pending}</b>}</a>)}</div>
