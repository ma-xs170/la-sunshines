...
export default async function OrganizerDetailPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return (
    <>
      ...
      <div className="org-subnav" role="navigation" aria-label="Sections">{TABS.map(([k, v]) => <Link key={k} href={`?onglet=${k}`} className={'org-subnav__link' + (tab === k ? ' is-active' : '')} aria-current={tab === k ? 'page' : undefined}>{v}</Link>)}</div>
      {tab === 'apercu' && (<>
