...
// Règles identiques au back-office : prix ≥ 0,50 €, quantité ≥ vendus + réservations en cours, tarif archivé non modifiable.
export async function PUT(req: Request, { params }: { params: Promise<{ slug: string }> }) {
  ...
  const { data, error } = await createSupabaseAdminClient().rpc('org_save_tier', {
    ...
    p_is_active: v.is_active, p_sort_order: v.sort_order,
  });
  if (error) {
