...
export async function GET(req: Request) {
  ...
  type Row = { id: string; status: string; holder_first_name: string; holder_last_name: string; used_at: string | null; ticket_tiers: { name: string; price_cents: number } | null; orders: { order_number: string; buyer_email: string; buyer_first_name: string; buyer_last_name: string; buyer_phone: string; source: string } | null };
  const rows = await fetchAll<Row>((from, to) =>
    db.from('tickets').select('id, status, holder_first_name, holder_last_name, used_at, ticket_tiers(name, price_cents), orders(order_number, buyer_email, buyer_first_name, buyer_last_name, buyer_phone, source)')
  ...
  const csv = toCsv(
    ['Billet', 'Statut', 'Prénom', 'Nom', 'Tarif', 'Type', 'Commande', 'Origine', 'Email acheteur', 'Prénom acheteur', 'Nom acheteur', 'Téléphone acheteur', 'Entré le'],
    rows.map((t) => [t.id.slice(0, 8), STATUS[t.status] ?? t.status, t.holder_first_name, t.holder_last_name, t.ticket_tiers?.name, t.ticket_tiers?.price_cents === 0 ? 'Gratuit' : 'Payant', t.orders?.order_number, t.orders?.source === 'manual' ? 'Invitation' : 'Achat', t.orders?.buyer_email, t.orders?.buyer_first_name, t.orders?.buyer_last_name, t.orders?.buyer_phone, t.used_at ? formatGp(t.used_at) : '']),
  );
