...
export default async function NextSteps({ slug, userId, hasTiers, published, fresh, hasFlyer }: { slug: string; userId: string; hasTiers: boolean; published: boolean; fresh: boolean; hasFlyer: boolean }) {
  const [det, row] = await Promise.all([
  ...
  const steps = nextSteps({
    hasDescription: Boolean(d?.description?.trim()), hasDresscode: Boolean(dress && ((dress.colors?.length ?? 0) > 0 || dress.free || dress.note)), hasFlyer: hasFlyer || Boolean(det.ok && det.data.media.length),
