      ...
      {s && (
        <>
          ...
          <table className="ord"><thead><tr><th>Tarif</th><th>Vendus</th><th>En réservation</th><th>Stock</th><th>Remplissage</th><th>Recette billets</th></tr></thead>
            <tbody>{s.tiers.map((t) => <tr key={t.tier_id}><td>{t.name}{t.archived && ' (archivé)'} — {formatEuro(t.price_cents)}</td><td>{t.sold}</td><td>{t.reserved}</td><td>{t.quantity_total}</td><td>{t.fill_rate} %</td><td>{formatEuro(t.revenue_cents)}</td></tr>)}</tbody></table>
        </>
      )}
    </div>
  );
