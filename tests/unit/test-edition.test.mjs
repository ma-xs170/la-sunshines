...
test('sans mode de test : aucun événement de test (le drapeau est faux en production, voir force-mode)', () => {
  assert.deepEqual(withTestEvent(other, false), other);
});
test('avec le mode de test : l\'événement « test-billetterie » est ajouté', () => {
  const r = withTestEvent(other, true, NOW);
...
test('content.json prioritaire : pas de doublon', () => {
  ...
  assert.deepEqual(withTestEvent(mine, true), mine);
});
