...
try {
  ...
  ok(/Non visible du public/i.test(list), 'un événement en brouillon désactivé affiche « Non visible du public »');
  ok(/Brouillon/i.test(list) && /Billetterie désactivée/i.test(list), 'raisons explicites : statut Brouillon + billetterie désactivée');
  ...
  await page.waitForURL(/\/admin\/contenu\?edit=/);
  const block = page.locator('section[aria-label="Billetterie"]');
  await block.waitFor();
  await page.waitForSelector('.tb-status');
  ok(/Non visible du public/i.test(await page.locator('.tb-status').innerText()), 'le bloc s’ouvre directement sur l’événement, bandeau « Non visible du public »');
  const st = await page.locator('.tb-status').innerText();
  ok(/Billetterie activée : NON/i.test(st) && /Statut : Brouillon/i.test(st), 'réglages visibles : « Billetterie activée : NON », « Statut : Brouillon »');
  ...
  ok(/Modifications non enregistrées/i.test(await block.innerText()), 'changement signalé « non enregistré »');
  ...
  await page.waitForFunction(() => /Configuration prête/i.test(document.querySelector('.tb-status')?.textContent ?? ''));
  ...
  ok(/Billetterie activée : OUI/i.test(await page.locator('.tb-status').innerText()), 'bandeau : « Billetterie activée : OUI », « Configuration prête »');
  ...
  ok(/Visible du public/i.test(list2) && !/Non visible du public/i.test(list2), 'la liste indique « Visible du public »');
  ...
  await page.waitForFunction(() => /Non visible du public/i.test(document.querySelector('.tb-status')?.textContent ?? ''));
  ...
  ok(/Non visible du public/i.test(list3) && /Aucun événement éditorial/i.test(list3) && !/Ouvrir l’évènement dans l’admin/i.test(list3), 'événement sans page éditoriale : message clair, pas de lien mort');
