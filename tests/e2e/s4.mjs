...
ok((m.attachments ?? []).length === 2, `${(m.attachments ?? []).length} QR en pièces jointes`);
ok(/cid:qr-0/.test(m.html) && /cid:qr-1/.test(m.html) && (m.attachments ?? []).every((a) => /^qr-/.test(a.content_id ?? a.contentId ?? '')), 'images référencées par Content-ID (compatible Gmail)');
...
const decoded = (m.attachments ?? []).map((a) => decodeQr(attBuf(a)));
