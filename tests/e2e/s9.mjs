...
if (hasPoppler) {
  ...
  ok(text.includes(ref) && /293 B/.test(text) && /THE MOUV/.test(text) && /104 253 943 00013/.test(text) && /Élodie/i.test(text), `texte du PDF : référence ${ref}, TVA 293 B, organisateur + SIRET, titulaire`);
...
ok(r.status === 200 && /Accès réservé/.test(r.data), 'un simple client voit « Accès réservé »');
...
r = await staff.req(`/organisateur/evenements/${A}?status=used`);
ok(/Élodie/.test(r.data) && !/Denis/.test(r.data), 'filtre par statut « Entré »');
r = await staff.req(`/organisateur/evenements/${A}?sort=name&dir=asc`);
...
r = await staff.req(`/organisateur/evenements/${A}?tier=nimporte&status=piege&sort=DROP`);
