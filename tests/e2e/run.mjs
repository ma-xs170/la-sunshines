...
let failed = false;
for (const s of process.env.ONLY ? process.env.ONLY.split(',') : ['s3', 's4', 's5', 's6', 's7', 's8', 's9', 's10', 's11', 's12', 's13', 's14', 's15', 's16', 's17', 's18', 's19', 's20', 's21', 's22']) {
  console.log(`\n### ${s}`);
  const r = spawnSync('node', [path.join(HERE, s + '.mjs')], { encoding: 'utf8' });
  try { fs.writeFileSync(`/tmp/e2e-${s}.out`, (r.stdout || '') + '\n' + (r.stderr || '')); } catch {}
