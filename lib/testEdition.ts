...
export function withTestEvent(events: StoredEvent[], env: Record<string, string | undefined> = process.env, now = Date.now()): StoredEvent[] {
  if (!testEditionEnabled(env) || events.some((e) => e.slug === TEST_EDITION_SLUG)) return events;
