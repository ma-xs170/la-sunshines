...
export interface AdminShellData { userId: string; firstName: string; reference: string | null; isSuper: boolean; pending: number; support: number }
...
export async function getAdminShellData(): Promise<AdminShellData | null> {
  ...
  const [acc, prof, pend, sup] = await Promise.all([
  ...
  return { userId: s.userId, firstName: s.profile.first_name, reference: (prof.data?.admin_reference as string | null) ?? null, isSuper: acc.data?.level === 'super', pending: pend.count ?? 0, support: sup.count ?? 0 };
}
